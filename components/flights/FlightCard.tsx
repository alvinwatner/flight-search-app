...
export function FlightCard({ flight }: FlightCardProps) {
  ...
  return (
    <Card ref={cardRef} hoverable className="p-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="flex-1 w-full">
          <div className="flex items-center gap-3 mb-4 flex-wrap">
            <Badge variant="info" size="sm">{flight.provider}</Badge>
          ...
          <div className="grid grid-cols-3 gap-6 mt-4">
            <div>
              <div className="text-2xl font-semibold">{formatTime(flight.departure)}</div>
              <div className="text-sm font-bold text-gray-900">{flight.origin.code}</div>
              <div className="text-xs text-gray-500">{flight.origin.city}</div>
            </div>

            <div className="text-center">
              <div className="text-sm font-medium text-gray-700">{formatDuration(flight.duration)}</div>
            ...
            <div className="text-right">
              <div className="text-2xl font-semibold">{formatTime(flight.arrival)}</div>
              <div className="text-sm font-bold text-gray-900">{flight.destination.code}</div>
        ...
        <div className="text-right w-full md:w-auto">
          ...
          <Button variant="primary" className="w-full md:w-auto" onClick={handleSelect}>
            Select
          </Button>
        </div>
      </div>
